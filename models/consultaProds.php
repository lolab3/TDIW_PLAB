<?php
    function getProds($connexio,$cat){
        try {
            $consulta_productes = $connexio->prepare("SELECT id, nom, preu_actual, descripcio, unitats_stock, img FROM Productes WHERE categoria_id=" . $cat);
    ...
    function getAllProds($connexio){
        try {
            $consulta_productes = $connexio->prepare("SELECT categoria_id, id, nom, preu_actual, descripcio, unitats_stock, img FROM Productes");
            $consulta_productes->execute();
